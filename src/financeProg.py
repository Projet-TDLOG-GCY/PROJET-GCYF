...
def plot_yesterday_stock(symbol):
    ...
    if jour_semaine=="Monday":
        yesterday = datetime.now() - timedelta(days=3) #on prend les données du vendredi pour le lundi
    if jour_semaine=="Sunday" or jour_semaine=="Saturday":
        print("La bourse est fermé le week end")
        return

    market_open_time = yesterday.replace(hour=9, minute=30, second=0) # heure d'ouverture du marché
